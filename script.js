...
function renderProjects() {
  ...
  $("#project-grid").innerHTML = visible.map((p, i) => `
    <article class="project" data-index="${PROJECTS.indexOf(p)}">
      <div class="project-image">${imageMarkup(p)}<div class="project-overlay"></div></div>
      <div class="project-info">
        <div><h3 class="project-title">${p.title}</h3><div class="project-meta">${p.category}</div></div>
        <div class="project-arrow">↗</div>
      </div>
    </article>
  `).join("");

  $$(".project").forEach(card => card.addEventListener("click", () => openModal(Number(card.dataset.index))));
}
